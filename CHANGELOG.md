...
### Added
- **CI release pipeline.** `.github/workflows/release.yml` picks up `v*.*.*.*` tags, builds + tests, signs the MSIX with the repo signing cert when `SIGNING_PFX_BASE64` / `SIGNING_PFX_PASSWORD` secrets are configured (public `.cer` tracked at `scripts/signing/noteshard-signing.cer`), and creates a GitHub Release with the MSIX + cert attached so winget URLs resolve. When the signing secrets are absent, the release gracefully falls back to an unsigned MSIX (filename suffixed `-unsigned`) so distribution still works. `ci.yml` builds Release (not Debug) and runs both test projects on every push + PR; the resulting unsigned MSIX is attached as a PR artifact for reviewer smoke-tests. `scripts/verify-versions.ps1` gates every CI run on the four version strings agreeing, and every release on the tag matching those versions.
...
- **Signing cert public half** at `scripts/signing/noteshard-signing.cer`. Users install it once into `Trusted People` to sideload signed releases.

### Changed
- **Contribution workflow: console-cowboy mode.** Direct pushes to `main` are the maintainer's default; branch protection is intentionally off. PRs from outside contributors remain welcome (the PR template + CODEOWNERS + CI checks still fire). `CONTRIBUTING.md` updated accordingly.
